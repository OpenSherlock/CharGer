...
public class KnowledgeManager {
    ...
    public Action summarizeKnowledgeAction = new AbstractAction() {
        public Object getValue( String s ) {
            if ( s.equals( Action.NAME ) ) {
                return charger.Global.strs( "SummarizeKnowledgeLabel" );
            }
     ...
     * that the matcher will be in the "kb" package, unless the name contains a
     * period (".")
     */
    public AbstractTupleMatcher createCurrentTupleMatcher() {
        ...
        if ( !className.contains( "." ) ) {
            className = "kb.matching." + className;
        }
        try {
            Class matcherClass = Class.forName( className );
            matcher = (AbstractTupleMatcher)matcherClass.newInstance();
        } catch ( ClassNotFoundException cnfe ) {
        	charger.Global.error( "Class " + className + " not found." );
        } catch ( InstantiationException ie ) {
        	charger.Global.error( "Class " + className + " couldn't be instantiated." );
        } catch ( IllegalAccessException iae ) {
        	charger.Global.error( "Class " + className + " couldn't be accessed." );
        }
        return matcher;
    }
